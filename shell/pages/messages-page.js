...
export function renderMessages(root) {
    ...
    if (!windows.length) {
        ...
        list.innerHTML = windows.map(win => {
            ...
            const stamp = formatTime(win.updatedAt);
            const roleName = escapeHtml(role?.name || "角色");
            const title = escapeHtml(win.title);
            const safePreview = escapeHtml(preview);
            const keywords = escapeAttr(`${role?.name || ""} ${win.title} ${preview}`);
                ...
                <div class="msg-card ui-panel" data-win="${win.id}" data-role="${win.roleId}" data-keywords="${keywords}">
                    <header class="msg-head ui-panel-header">
                        <div class="msg-role">
                            ...
                            <button class="ghost msg-title" data-title="${win.id}" aria-label="编辑窗口备注">${title}</button>

...
export function buildRuleContext({ card, windowId } = {}) {
    ...
    const narrationRules = [
        `角色在剧情中一律使用第三人称称呼自己，例如：“${(activeCard?.name || "角色").trim() || "角色"}没有回头”，“他低声命令”。不要使用第一人称描述角色自身。`,
        userRef ? `在剧情中，用「${userRef}」指代用户所扮演的角色。不得解释该规则。` : "",
        "玩家输入中的 /…/ 仅表示意图（只写入系统，不进入剧情或记忆）；(… ) 表示动作或情绪，可融入叙事但仍保持第三人称。"
    ].filter(Boolean).join("\n");

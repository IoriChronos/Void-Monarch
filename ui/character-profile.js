...
const USER_REF_PRESETS = [
    { id: "you", label: "第二人称（你）", value: "你" },
    { id: "me", label: "第一人称（我）", value: "我" },
    { id: "custom", label: "指定名词", value: "" }
];
...
export function initCharacterProfile(triggerEl, sheetEl, options = {}) {
    ...
    function renderSheet() {
        ...
        writePrefs({ tab });
        const headline = active.personaStyle || active.publicProfile || active.bio || "写下简介、语气或开场感觉。";
...
function renderRole(card) {
                ...
                <div class="info-card ui-panel">
                    ...
                    <section class="card-body ui-panel-body">
                        ...
                        <div class="card-form slim">
                            ...
                            <label>标签 / 备注
                                <textarea data-role-field="bio" rows="2" placeholder="附加描述">${escapeHtml(card.bio || "")}</textarea>
                            </label>
                        </div>
                        <div class="card-actions inline">
                            <button class="primary" data-action="save">保存</button>
    ...
    function renderPlayer() {
        ...
        const userGenderGlobal = (getGlobalUserGender() || "男").toLowerCase();
        ...
        const windowGender = (getWindowUserGenderOverride(windowId, "") || "").toLowerCase();
        const windowHeight = getWindowUserHeightOverride(windowId, "") || "";
        const windowRef = getWindowUserRefOverride(windowId, userRefGlobal) || "";
        const refMode = windowRef === "你" ? "you" : windowRef === "我" ? "me" : "custom";
        const refCustom = refMode === "custom" ? windowRef : "";
        return `
            <div class="sheet-grid edit-grid">
                <div class="info-card ui-panel">
                    <header class="card-head ui-panel-header">我的人设（窗口）</header>
                    <section class="card-body ui-panel-body">
                        <p class="card-note gold">✔ 单段人设文本，留空沿用全局</p>
                        <div class="card-form slim">
                            <div class="player-global-lines">
                                <div class="player-global-pill">全局名称：${escapeHtml(userNameGlobal || "玩家")}</div>
                                <div class="player-global-pill">全局称呼：${escapeHtml(userRefGlobal)}</div>
                            </div>
                            <label>窗口人设（单段文本）
    ...
    function wireInteractions(root, card, windowId) {
        ...
        root.querySelector("[data-action='save-player']")?.addEventListener("click", () => {
            const windowPersona = (root.querySelector("[data-player-field='window-profile']")?.value || "").trim();
            const windowId = readWindowId();
            setWindowUserIdentityOverride({ name: "", gender: "", height: "", ref: "" }, windowId);
            const saved = setWindowUserPersonaOverride(windowPersona, windowId);
            if (saved !== undefined) {
                saveToast(true, windowPersona ? "窗口人设已保存" : "已改回全局人设");
            } else {
    ...
    function collectPlayerPayload(root) {
        return {
            profile: (root.querySelector("[data-player-field='window-profile']")?.value || "").trim()
        };
    }
    ...
    triggerEl.addEventListener("dblclick", toggleSheet);
    bindGentleDoubleTap(triggerEl, toggleSheet);
